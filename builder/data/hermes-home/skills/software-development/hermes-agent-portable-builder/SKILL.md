...
- hermes update 依赖安装失败（cryptography 48→50 安全升级；2026-08-15 修正归因，推翻 08-14 的"自身进程锁"判断）：根因是 `cryptography/_rust.pyd` 被**运行中的进程**占用——Windows 拒绝替换被加载的 DLL（os error 5）。进程模块实测：桌面后端 `python -m hermes_cli.main serve` 加载了 `_rust.pyd`/`_cffi_backend.pyd`；`import hermes_cli.main` 本身**不**加载 cryptography（排除"update 进程锁自己"）；无桌面场景的锁来自 gateway/残留 python/Defender 类进程。**停进程有效**：stage 上无进程时 48→50 升级一次成功（uv 实测 exit 0）。FIXED (2026-08-15) in Update.cs: `StopPortableProcesses(root)`（按 root 路径过滤停 Hermes/python，不误杀其他 python）执行两次——官方 hermes update 前（解 cryptography 锁）与 Repair `-UpdatePython` 前（解 hermes update 残留子进程对 venv 的锁，Rename-Item venv 曾 Access denied）。同日删除 hermes update 失败重试逻辑（用户决定：重试对真实故障无益）。Symptoms: failure in `hermes update`'s `uv pip install -e .` (official code path, not portable), venv with two cryptography dist-info leftovers, Repair 的 Rename-Item venv Access denied。
...
Prefer this direct interpreter/module path over generated console-script shims. Avoid editable installs that point back to the 构建源码.
...
Give every Portable archive a collision-free local build timestamp after the platform suffix. Use `Hermes-Agent-Portable-<version>-win-x64-yyyyMMdd-HHmmss.zip`, based on the builder's local time at archive creation. Do not overwrite or ambiguously reuse an older artifact with the same Hermes version; validate the timestamped filename as part of release inventory checks.
...
- mutates the Git 源码 or rebuilds Electron while Desktop/backend processes still hold files;
...
4. **Portable Desktop rebuild/sync:** reapply the bounded early portable-path patch to the freshly updated Electron source, build `win-unpacked`, validate `Hermes.exe`, stage a complete `app.portable-next`, then atomically swap it with the live `app` while retaining rollback material. Patch apply/remove must be idempotent; remove (`-Stage PatchRemove`) should succeed when the patch or Desktop source is already absent. Since 2026-08-08 the sync stage self-cleans: after the swap it runs the patch-remove stage itself, so the 内嵌源码 is pristine when the script exits and a direct `hermes update` (outside Update.exe) never hits the "Restore local changes now? [Y/n]" stash prompt; Update.exe's own pre-update `-Stage PatchRemove` (and its `git clean -fd`) remains as a safety net for patches from other sources. The three update-chain scripts (repair / patch / sync) were merged into one `<portable-root>\scripts\Update-Portable.ps1` on 2026-08-10 (stages now `-Stage Patch|PatchRemove|SyncDesktop|WriteDesktopStamp`; the `Repair` stage was split back out into the standalone `Repair-Portable.ps1` the same day).
...
3. remove backend user state such as `.env`, `auth.json`, `state.db*`, sessions, logs, memories, caches, snapshots, profiles, pairing state, and user-created skills/plugins unless the distribution explicitly intends to seed them. There are NO intentional seed files under `builder\data\hermes-home\memories\` as of 2026-08-13 (a USER.md user-profile seed was documented on 2026-08-12 and has since been removed — do not re-add it); the single `builder\data` → `data` `Copy-Tree` block means anything added under `builder\data` ships as-is, so that tree is also the place to ADD a deliberate seed if one is ever wanted. Do not add `MEMORY.md` (agent personal notes) as a seed — it carries build-machine-specific state (DNS/hosts diagnostics, project-internal records) that must not ship to every user. The `hermes-agent-portable-builder` user-skill presence/slim gates were removed from `Test-PortablePythonContract` on 2026-08-12 at the user's request (skills are now optional in the release); the remaining skill-related gate (`git ls-files` overlay check in the 内嵌官方源码) stays, but since 2026-08-19 it **enumerates `builder\data` dynamically** (strips the leading `hermes-home/` segment and checks every seed file against the embedded checkout's index) instead of hardcoding a skill name — renaming/adding/removing a seed never needs a script edit. Verified both ways: 358 seed files, 0 tracked (clean), fake overlay file → caught.
4. retain only required runtimes/source plus a minimal non-secret default config (for example, a language default). For Hermes Agent Desktop Portable, the release ships WITHOUT `data/hermes-home/config.yaml` — the stage is assembled from an UNLAUNCHED upstream 源码, so the file never exists in the normal flow (the app creates it only on first launch); the build still fail-closes around it: an early defensive `$ConfigPath` remove + throw in `Hermes.ps1` and a final absence re-check in `Test-PortablePythonContract` (`Test-Path $UserConfig` throw, reported as `UserConfigPackaged=false` / `FirstRunLanguage='zh'` / `ExistingConfigPreserved=true`). The Chinese default is a FIRST-RUN launcher seed written only when the file is absent: `Hermes.cs::EnsureFirstRunConfig` (`FileMode.CreateNew`, writes `display:\n  language: zh\n`) and the `Update-Portable.ps1` SyncDesktop stage (`fs.writeFileSync(..., flag:'wx')`, EEXIST ignored) — an existing user config is never overwritten. Do NOT add config.yaml to `builder\data`: the early removal runs BEFORE the builder-data `Copy-Tree`, so a builder-data copy would survive that removal and be caught only by the contract gate's final absence check (build fails); do not change upstream `DEFAULT_LOCALE` — the first-run seed is the distribution default and later user language choices persist;
...
- Shallow-clone packaging (2026-08-23): the portable's embedded checkout now ships as a depth-1 shallow .git (`Convert-PackagedGitToShallow` in Hermes.ps1) — full history 737MB → ~69MB, and official `hermes update` handles shallow checkouts (verified 2026-08-23 END-TO-END on a moved root: real-network `hermes update --check` fetched origin/main via `--depth 1` and kept is-shallow=true at 69M; then a local fixture repo (one new commit on top of the packed HEAD) drove a full `hermes update --yes` — fetch + `git merge --ff-only origin/main` (update_cmd.py:6212-6213) fast-forwarded HEAD, the `.git/shallow` boundary file stayed unchanged, .git stayed 69M, `git status` clean, `hermes --version` healthy, no `.update-incomplete` marker. Source evidence for the --depth 1 fetch: update_cmd.py:3196-3205 picks `depth_args`, :3225/:3237 pass it to `git fetch`; official docs say nothing about shallow). Verified sequence: `git fetch --depth 1 --no-tags <local-upstream> main` → `git reset --hard FETCH_HEAD` → `git remote remove origin` + `git remote add origin <url>` → delete ALL local tags → `git reflog expire --expire=now --all` → `git gc --prune=now --aggressive`. PITFALL 1: `git clone --depth 1 <local-path>` is silently IGNORED by git's --local hardlink optimization (produces full history, is-shallow=false) — must use fetch --depth 1. PITFALL 2: the repo carries ~1600 origin remote-tracking branch refs plus version/backup tags that reference history objects; gc cannot prune them — drop the origin remote (re-add by URL) and delete all tags FIRST, then gc. PITFALL 3: `git fetch --depth 1 <local-path> main` from MSYS bash needs Windows forward-slash paths (`C:/...`), and in PowerShell source use `$Repo.Replace([char]0x5C,[char]0x2F)` instead of a backslash literal (patch-tool double-backslash trap). PITFALL 4 (2026-08-23, three failed builds): Hermes.ps1 is authored NO-BOM UTF-8, and PowerShell 5.1 decodes no-BOM files with the ANSI/GBK code page. A Chinese comment whose UTF-8 bytes leave a dangling 0x80-0xBF byte swallows the following newline: adjacent comment lines merge AND the next code line is absorbed into a `#` comment and silently never runs. Observed: the `$localRepo = $Repo.Replace(...)` line vanished into a merged comment (Get-Content line count 1077 vs true 1085); `$localRepo` stayed $null, and `git fetch --depth 1 --no-tags <empty> main` failed with `fatal: 'main' does not appear to be a git repository` (git reads 'main' as the repository argument). FIXED by prepending the UTF-8 BOM (EF BB BF) to Hermes.ps1 — PowerShell then decodes UTF-8 correctly and every code line survives. VERIFY after adding Chinese comments to a no-BOM .ps1: compare PowerShell `Get-Content -Raw` line count with the true count and confirm the code line right after a comment block is standalone. Hermes.ps1 MUST keep its BOM; re-add it after any future copy from the D: original (which stays no-BOM). Verified after shallow: `git rm -r --cached` + `reset --hard` (line-ending normalization) keeps status clean and shallow state intact; `git fetch --depth 1 origin main` + `git merge --ff-only origin/main` (official update path) stays incremental at 69MB.
- Upstream can track two paths that differ only by case (observed 2026-08-17: `contributors/emails/agent@Agents-Mac-mini.local` vs `agent@agents-Mac-mini.local` — upstream commit 55db6187e added the upper-case variant while the lower-case one already existed, both in origin/main with DIFFERENT contents: `skip-agent` vs `momomojo`). On Windows' case-insensitive filesystem only one can materialize in the working tree, so `git status --porcelain` ALWAYS reports the other as modified, and `Hermes.ps1`'s release gates throw "Embedded checkout is dirty before release." (2026-08-17 build failure, line 744) / "Packaged checkout is not clean". FIXED in the build script: `Protect-CaseCollisionEntries` (defined after `Invoke-NativeChecked`, ~L464) lists tracked files, groups by `ToLowerInvariant()`, and for each group marks `update-index --skip-worktree` every entry whose `HEAD:<path>` blob differs from the working-tree `hash-object` (the one Windows actually materialized). It is called BEFORE both status gates (staged ~L771 and packaged ~L826). WHY TWO CALL SITES: the flag lives per-entry inside `.git\index` (shown as `S` by `git ls-files -v`), NOT in git config. A plain `git reset --hard` PRESERVES it (verified 2026-08-18), but the packaged-checkout block replaces the whole `.git` with a fresh copy from upstream (`Remove-TreeSafe` + `Copy-Tree` ~L782-784 — that index carries no flags, upstream is a read-only mirror never marked) and then `git rm -r --cached .` + `git reset --hard` (~L796-797) rebuilds the index FROM HEAD, dropping any surviving flag — so the packaged gate must re-run the protection after the index rewrite. Each call re-checks both variants because which one Windows materializes can differ per index rebuild (observed: staged check hid the lower-case entry, packaged check hid the upper-case). Bonus: the flag ships inside the packed `.git` index, so the user-side `hermes update` local-changes check also stays clean (same failure class as the CRLF trap). When upstream eventually deletes one variant, the loop no-ops. Do NOT try to "fix" this in the upstream checkout — it must stay a byte-exact mirror of origin/main.
- `npm install --prefix <nodeDir> npm@<major>` (Ensure-OfficialNpm in Hermes.ps1 ~L422, and its inline twin in Update-Portable.ps1 ~L428) PRUNES the official Node zip's bundled `node_modules\corepack` (verified 2026-08-14 by reproduction: the npm 10.9.8→11.19.0 upgrade left only `.bin`/`.package-lock.json`/`npm` in node_modules; the leftover corepack/corepack.cmd shims then fail with MODULE_NOT_FOUND — same symptom as the live portable's dead corepack). FIXED (2026-08-14, reworked same day per user preference — a backup/restore variant was proposed first and rejected by the user) in BOTH scripts: read the bundled corepack version from `node_modules\corepack\package.json` BEFORE the npm upgrade, then after the upgrade reinstall exactly that version (`& $npmCmd install --prefix $NodeDir "corepack@$corepackVersion" --no-fund --no-audit --progress=false` when `dist\corepack.js` is missing). Reading the version from the unpacked tree means it always follows the official Node zip — no hardcoded version anywhere. Verify: end-to-end repro with the real zip — read version 0.34.6 → npm 10.9.8→11.19.0 upgrade (corepack pruned) → reinstall corepack@0.34.6 → npm 11.19.0 + corepack 0.34.6 + `corepack pnpm --version` = 11.7.0. The LIVE portable's node lost corepack the same way (installed from a build artifact); fixed there with `npm install -g corepack` — note npm's global install may EEXIST on leftover shims (corepack's bin aliases pnpm/pnpx/yarn/yarnpkg clash), so uninstall any global pnpm first or reinstall with --force. Regression gate (same day): Verify-Portable.ps1 now checks `$Checks.Corepack` (node\node_modules\corepack\dist\corepack.js) + `$Checks.CorepackShim` (node\corepack.cmd) existence (missing → exit 1, build fails) and reports `$results.Corepack` via `& $Checks.CorepackShim --version`. Verified: live passes (`Corepack: 0.35.0`), the corepack-less stage fails with "Missing Corepack" exit 1.
- Desktop incremental build (2026-08-14): Update-Portable.ps1 SyncDesktop skips the electron-builder rebuild when the desktop tree did not change since the packaged build. Mechanism: content hash of `apps/desktop` (git ls-files + `--others --exclude-standard`, mirroring the official hermes_cli `_compute_desktop_content_hash` walk) + root package.json/package-lock.json, stored in `data\hermes-home\desktop-build-stamp.json` — written by Hermes.ps1 (via Update-Portable.ps1 `-Stage WriteDesktopStamp`) and by SyncDesktop after a successful build + PatchRemove (pristine, patch-free state so both sides compare the same bytes). PITFALL (found 2026-08-14, same class as the web stamp): the build-machine stamp MUST be written AFTER the staged line-ending normalization (`git rm --cached` + `reset --hard`, which rewrites every tracked file to the .gitattributes endings). Written before it, the stamp hashes CRLF/legacy bytes (verified: `8cc565cc` vs `1082986a` for the same tree) while the deployed side sees LF — the incremental check then always reports stale and every update rebuilds. When the hash matches, SyncDesktop keeps the current app (no swap, no patch round-trip) and only rebuilds TUI/Web bundles; the swap/PatchRemove/stamp block is skipped. If upstream ever changes what feeds the desktop build, extend BOTH Get-DesktopContentHash (Update-Portable.ps1) and the official _compute_desktop_content_hash — the comparison only works when both sides hash the same file set. TUI/Web bundles use the OFFICIAL judges (no reimplementation): `_tui_need_rebuild(ui-tui)` (mtime, via the live venv python) and `_web_ui_build_needed(web)` (content-hash stamp) gate the npm install + build; errors default to rebuild (conservative). Verified on stage: `_web_ui_build_needed -> False` (stamp matches → skip works), `_tui_need_rebuild -> True` (fresh copy → rebuild).
...
- Update.cs full review (2026-08-15, first five fixed in the builder source + recompiled into stage AND deployed Update.exe, byte-verified: new UTF-16LE literals present / old ones absent, stage vs deployed 0 differing bytes that compile; the sixth change is template-only — the user deferred recompiling the deployed Update.exe, syntax-gated via csc to %TEMP% only with the artifact removed, so the next full build picks it up): (1) FAIL-CLOSED gate aborts (`Fail("进程占用", …)` / `Fail("防重入", …)`) passed rc=0, so `Finish(0)` wrote `"ok":true` into `.hermes-update-result.json` — Hermes.cs::ShowUpdateResultIfFailed only pops when ok != true, so a missed MessageBox meant a silently lost failure, and Update.exe exited 0 for an abort. rc is now 1 on both gates. (2) `StopPortableProcesses` used `$_.Path -like '<root>*'` with the TrimEnd'd root — the SAME prefix-collision bug class already fixed in IsForeignServe: it killed Hermes/python of OTHER installs whose path merely begins with this root (`...-Portable-2\`, `...-Portable-Beta\`). Now `$_.Path.StartsWith('<root>\', [System.StringComparison]::OrdinalIgnoreCase)` (directory boundary; also immune to `-like`'s `[` wildcard). (3) The standalone-uv fallback (`uv pip install --python <venv> -e .` after a failed `hermes update`) reset rc=0 on uv success even when the git step failed (network/conflict) — a fake "Update complete." with un-updated source (uv succeeds offline when deps are already satisfied). Now the pre-update HEAD is recorded via `RunGitHead` (`git -C <repo> rev-parse HEAD`) and the fallback only clears rc when HEAD advanced. (4) `RunCaptured(cli, "-m hermes_cli.main update", …)` duplicated hermes-cli.cmd's own `-m hermes_cli.main` wrapper — it worked ONLY by accident (`-m` is the `--model` alias in hermes_cli/main.py and the parser uses parse_known_args, verified live with `update --check`). Now `RunCaptured(cli, "update", …)`. (5) `FindForeignDashboardPids` depended on wmic.exe (removed on Win11 24H2+) and its /FORMAT:LIST parser + `IsForeignServe`; now a single PowerShell `Get-CimInstance Win32_Process` pipeline with `-match 'serve|dashboard'` + `-notlike '<root>\*'` emitting bare pids, parsed from stdout. (6) `RunGitHead` streamed the `git rev-parse HEAD` hash to the update console via RunCaptured's real-time forwarding (user-visible noise, reported 2026-08-15). `RunCaptured` now has a 4-arg forwarding overload + 5-arg `silent` overload (silent still captures into the diag log, just skips Console forwarding); `RunGitHead` runs with silent=true. Verification details for (1)-(5): PS snippets exercised read-only against the live root (root Hermes/python matched, `Portable-2`/`Portable-Beta` prefixes NOT matched, foreign pids listed, root pids excluded) — the wmic-era "collect pid/cmd/exe trios" parser is gone.
- Official `upstream/scripts/desktop-update.ps1` (429 lines, landed 2026-08-10) is the repo-owned update hand-off the Desktop Update button spawns via a `cmd start` wrapper. It solves the FROZEN-BINARY problem (staged Tauri exe has no self-update path, so updater fixes only ship with new installers; a repo-owned script refreshes every `hermes update`). Its transferable ideas, all adopted into `Update.cs` on 2026-08-10: (a) FAIL CLOSED preflight gates — wait for the Desktop process to exit (30s) and for the venv shim to unlock (20s), aborting with nothing changed otherwise; the 2026-08-09 Access-denied brick came from updating under a live backend. Our Portable equivalent: `WaitForRootProcessesExit(root, 30)` scans `Process.GetProcesses()` for Hermes.exe/python.exe under the root (directory-boundary match) and aborts via the gate before touching the venv. (b) ONE retry of `hermes update` when exit != 0 && != 2 (2 = "close all Hermes windows", not retryable) — the update-boundary class (fresh code on disk, stale code in memory). REMOVED 2026-08-15 at the user's request (retrying does not fix real failures such as the cryptography DLL lock; it only doubles the wait). (c) Result-file hand-off: every exit path writes `data\hermes-home\.hermes-update-result.json` `{ok, exit_code, message, finished_at}` via a unified `Finish()` (also removes the re-entrancy marker); `Hermes.cs` `ShowUpdateResultIfFailed` reads+deletes it on next launch and pops a dialog when ok=false, so a detached/crashed update is never silent. (d) Re-entrancy marker `data\hermes-home\.hermes-update-in-progress` containing OUR pid, reclaimed when stale (pid dead), removed only while owned — prevents two concurrent Update.exe from racing over the same 源码/venv. Also official does UTF-8 console + PYTHONIOENCODING/PYTHONUTF8 (same as our fix), WMI-detached relaunch to avoid Electron AttachConsole console-inheritance, `AllowSetForegroundWindow` focus hand-off, and treats "Desktop build failed" inside an exit-0 `hermes update` as fatal (our Portable flow already avoids this: the SyncDesktop stage rebuilds Desktop itself and checks Hermes.exe size). Note the official script lands in the 打包源码 automatically (it is part of upstream), so it ships in every Portable — it is inert there (the Desktop Update button is not wired to Portable's Update.exe flow).
- The `__pycache__` cleanup of the staged hermes-agent source tree MUST run AFTER the web UI build stamp step: the stamp step imports `hermes_cli` through the staged venv (PYTHONPATH points at the 源码) and re-creates `__pycache__` under the source tree, so a cleanup placed before it lets fresh .pyc files slip into the archive (observed 2026-08-05: ZIP shipped `hermes-agent/.../__pycache__/*.pyc` stamped seconds before archive creation; fixed by moving the `Get-ChildItem $Checkout -Directory -Filter '__pycache__' ... | Remove-Item` block after the stamp `try/finally`). Archive inventory check: `7za l` — the only remaining `__pycache__` entries must be under `runtime\\python\\...\\Lib\\` (CPython's own precompiled stdlib bytecode, normal); any entry under `data\\hermes-home\\hermes-agent\\` means the cleanup ran too early.
- The Desktop stage of `Hermes.ps1` historically ran bare `npm run typecheck/build` with NO dependency install of its own (only TUI and web have `npm install --workspace` steps), relying on the build machine's leftover `node_modules`. Whenever upstream adds a Desktop dependency, the stale tree breaks the build with TS2307 (verified 2026-08-09: `get-windows@9.3.0` from the `read_window_below` tool failed typecheck until `npm.cmd install --workspace apps/desktop --include=dev --silent --no-fund --no-audit --progress=false` was added right after the Portable source patch apply, before typecheck; npm hoists workspace deps to the ROOT `node_modules` — check `upstream/node_modules/<pkg>/`, not `apps/desktop/node_modules/`). The later `git checkout -- package-lock.json` (after the TUI/web steps) still restores the lockfile, so the install step cannot dirty the 源码. HOISTING IS NOT GUARANTEED: as of 2026-08-10 the official root `package.json` no longer declares `electron` at all (it lives only in `apps/desktop` devDependencies, pinned 40.10.2), so `npm install --workspace apps/desktop` leaves it at `apps/desktop/node_modules/electron` and the root `node_modules/electron` does NOT exist — `Hermes.ps1` failed at the `$electronVersion` read (`Get-Content ... node_modules\electron\package.json` → PathNotFound, and the bash wrapper masked the real exit code behind `tail`). Fixed with a fallback lookup: root `node_modules\electron\package.json`, else `apps\desktop\node_modules\electron\package.json`, throw if neither. If upstream moves a dependency again, check the root `package.json` declaration before assuming where npm put a package.
...
- Stage cleanup timing (changed 2026-08-13 at user request — was delete-after-build, now delete-first): `Hermes.ps1` deletes the staged Portable directory at script start (`Remove-TreeSafe $Stage` + throw, right before the main flow's `Read-OfficialPythonSelector`) — targeting `stage\Hermes-Agent-Portable` ONLY, not the `stage\` parent (refined the same day at the user's request: the parent is a plain container expected to stay empty). This is the ONLY stage deletion; the pre-assembly re-check was removed at the user's request the same day. The Desktop/TUI/Web builds run entirely in the `upstream` checkout (→ `apps\desktop\release\win-unpacked`, `hermes_cli\tui_dist\entry.js`, `hermes_cli\web_dist`), and the stage is only written from assembly onward (`New-Item -ItemType Directory -Force $Stage`). Accepted trade-off (user-chosen): a failed build no longer preserves the previous staged tree for inspection, but an occupied-stage error now surfaces at script start instead of after the builds finish. Describe the flow as "构建开始前删除 stage 内的 Portable 目录（保留 stage 父目录）".
- A release archive WITHOUT `data\hermes-home\config.yaml` is CORRECT, not a missing seed (observed 2026-08-13 — reported as a "seed missing" false alarm before the user corrected the assumption). The stage is built from an UNLAUNCHED upstream 源码, so config.yaml never exists there in the normal flow; the build fail-closes around it anyway (early defensive `$ConfigPath` remove + throw in `Hermes.ps1`, plus `Test-PortablePythonContract`'s final `Test-Path $UserConfig` throw — `UserConfigPackaged=false` / `FirstRunLanguage='zh'` / `ExistingConfigPreserved=true`). The zh default is a FIRST-RUN launcher seed (`Hermes.cs::EnsureFirstRunConfig` `FileMode.CreateNew`, `Update-Portable.ps1` SyncDesktop `flag:'wx'` — both write `display:\n  language: zh\n` only when the file is absent). Absence in the ZIP is the designed state; a config.yaml in the LIVE deployment (`<portable-root>\data\hermes-home\config.yaml`) is first-run user state created by the launcher, preserved across updates — do not confuse it with a build artifact.
