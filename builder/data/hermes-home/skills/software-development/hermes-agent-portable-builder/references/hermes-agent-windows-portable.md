...
- `web-ui-build-stamp.json` (`$HERMES_HOME`): content hash of web/ source, written by the SAME code that reads it — `hermes_cli.main_web_build._write_web_ui_build_stamp(repo, repo/'web')` via the staged venv (`$env:HERMES_HOME` + `PYTHONPATH` pointed at the 暂存源码) — otherwise `_web_ui_build_needed()` is True on first launch and `hermes dashboard` runs a runtime npm install. Fatal in the packaging script; non-fatal in the desktop-sync helper (never delete a good `web_dist` because the stamp failed).
- Add `-e hermes_cli/web_dist/` to every staged `git clean -fdx` (web_dist is gitignored upstream; mirror `-e hermes_cli/tui_dist/`).

## Upstream hermes_cli/main.py split (2026-09-05, build fatal)
Official upstream split the `hermes_cli/main.py` god module into siblings `main_web_build.py` / `main_tui_launch.py` / `main_desktop.py` / `main_dashboard.py` / `main_agent_cmds.py` / `main_install_repair.py` / `main_provider_setup.py` / `main_platform_setup.py`. The three judge/stamp symbols the builder relies on moved out of `hermes_cli.main` and are NO LONGER importable from it (ImportError `cannot import name '_write_web_ui_build_stamp' from 'hermes_cli.main'`, Hermes.ps1 'Web UI build stamp' step exit 1): `_write_web_ui_build_stamp`/`_web_ui_build_needed`/`_stamp_is_current`/`_write_build_stamp`/`_hash_source_tree` now live in `hermes_cli/main_web_build.py`, `_tui_need_rebuild` in `hermes_cli/main_tui_launch.py`; `main.py` re-exports only a subset (`_build_web_ui`, `_launch_tui`, ...) plus the updater surface — NOT the private judges. FIXED in `Hermes.ps1` (web stamp probe) and `Update-Portable.ps1` SyncDesktop (TUI/Web probes + stamp): import from the defining module (`from hermes_cli.main_web_build import _write_web_ui_build_stamp` / `_web_ui_build_needed`; `from hermes_cli.main_tui_launch import _tui_need_rebuild`). Signatures/arguments unchanged (`_tui_need_rebuild(Path('<repo>\ui-tui'))`, `_web_ui_build_needed(Path('<repo>\web'))`, `_write_web_ui_build_stamp(Path('<repo>'), Path('<repo>\web'))`). The CLI invocation `python -m hermes_cli.main desktop --build-only --force-build` (SyncDesktop desktop rebuild) still resolves (desktop subcommand wiring unchanged). Verification: staged venv probe imports clean after the fix; a full Hermes.ps1 build passes the Web UI build stamp step. Lesson: when the upstream refactor moves a symbol, update BOTH builder-side call sites (Hermes.ps1 + Update-Portable.ps1) and their shipped copies (stage + deployed) — `hermes_cli.main` re-export coverage is not guaranteed.
