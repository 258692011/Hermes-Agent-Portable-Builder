...
`upstream/` is a read-only official checkout. NEVER use `git pull --ff-only` — it preserves local changes (e.g. stale patch residue) and silently diverges from upstream. `reset --hard` guarantees a byte-exact mirror. The build script will apply and remove its own portable patch; unrelated local modifications must not survive.
...
- Network-resilient upstream sync (observed 2026-08-12): if `git fetch` fails with "Failed to connect to github.com:443 ... Could not connect to server" while `api.github.com` / `codeload.github.com` / `raw.githubusercontent.com` still respond, the local DNS (hotspot/router, e.g. 192.168.43.1) is resolving `github.com` to an unroutable APAC edge (20.205.243.166 — IPv4 connect timeout, no IPv6 AAAA). Diagnose candidate edges with `curl --resolve github.com:443:<ip> --connect-timeout 8 https://github.com` (expect 200); verified working on this build machine: `140.82.112.3` and `140.82.114.3`. Fix: append `140.82.112.3 github.com` (and `140.82.114.3 github.com` as backup) to `C:\Windows\System32\drivers\etc\hosts`, then fetch as usual. WINDOWS HOSTS QUIRK: only the FIRST matching entry per hostname is used — no automatic failover; swap the lines to switch edges. Verify the fetched HEAD equals the API-reported main SHA (`curl -sS https://api.github.com/repos/NousResearch/hermes-agent/commits/main` and compare `"sha"`).
